...
while :
do
  [ "$(curl --location --request GET 'bev.domain.at/ajax/get_key')" != "notyet" ] && break
done

pass="$(curl --location --request GET 'bev.domain.at/ajax/get_key')"
...
curl --location --request GET 'bev.domain.at/ajax/set_key?password=notyet'
